...
async function drawImage(page: ReturnType<PDFDocument["addPage"]>, primitive: Extract<ScenePrimitive, { type: "image" }>, resources: PdfSceneResources): Promise<void> {
  ...
  const x = primitive.xMm * MM_TO_PT;
  const y = toPdfY(pageHeight, primitive.yMm * MM_TO_PT, primitive.heightMm * MM_TO_PT);
...
export async function renderScenePdf(scene: CardScene, resources: PdfSceneResources): Promise<Uint8Array> {
  assertRenderable(scene);
  const pdf = await PDFDocument.create();
  const page = pdf.addPage([scene.widthMm * MM_TO_PT, scene.heightMm * MM_TO_PT]);
  ...
  for (const primitive of scene.primitives) {
    if (primitive.type === "rect") {
      page.drawRectangle({ x: primitive.xMm * MM_TO_PT, y: toPdfY(page.getHeight(), primitive.yMm * MM_TO_PT, primitive.heightMm * MM_TO_PT), width: primitive.widthMm * MM_TO_PT, height: primitive.heightMm * MM_TO_PT, color: color(primitive.fill) });
      ...
      const boxX = primitive.xMm * MM_TO_PT;
      const width = primitive.widthMm * MM_TO_PT;
      const x = primitive.align === "left" ? boxX : primitive.align === "center" ? boxX + (width - textWidth) / 2 : boxX + width - textWidth;
      page.drawText(primitive.text, { x, y: toPdfY(page.getHeight(), primitive.baselineMm * MM_TO_PT), size: primitive.sizePt, font, color: color(primitive.color) });
    } else await drawImage(page, primitive, resources);
  }
