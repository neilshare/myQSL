import { AnyCardTemplateSchema, PrintManifestSchema, type PrintManifestV1 } from "@myqsl/domain";
...
export class PrintService {
  ...
  async create(request: PrintRequest, idempotencyKey: string): Promise<{ manifest: PrintManifestV1; replayed: boolean }> {
    ...
    const hasV2 = items.some((item) => item.template_schema_version === 2);
    const base = { schema_version: 1 as const, batch_id: batchId, kind: request.kind, profile, renderer_version: hasV2 ? "pdf-v2" : "pdf-v1", font_manifest_version: hasV2 ? "fonts-2026-09-14" : "fonts-v1", items, created_at: createdAt, expires_at: expiresAt };
    const manifestHash = await sha256(stable(base));
    const manifest = PrintManifestSchema.parse({ ...base, manifest_hash: manifestHash });
    const statements: D1PreparedStatement[] = [this.env.DB.prepare("INSERT INTO print_batches(id,idempotency_key,request_hash,kind,profile_json,status,renderer_version,font_manifest_version,manifest_hash,created_at,expires_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)").bind(batchId, idempotencyKey, requestHash, request.kind, JSON.stringify({ profile }), "ready", base.renderer_version, base.font_manifest_version, manifestHash, createdAt, expiresAt)];
    for (const item of items) {
      statements.push(this.env.DB.prepare("INSERT INTO print_batch_items(batch_id,position,qso_id,card_id,snapshot_json,snapshot_hash,background_asset_id,background_sha256,public_url,qr_omitted,asset_refs_json) VALUES(?,?,?,?,?,?,?,?,?,?,?)").bind(batchId, item.position, item.qso_id, item.card_id, item.snapshot_json, item.snapshot_hash, item.background_asset_id, item.background_sha256, item.public_url, item.qr_omitted ? 1 : 0, JSON.stringify(item.asset_refs ?? [])));
      for (const assetId of item.asset_refs ?? []) statements.push(this.env.DB.prepare("INSERT OR IGNORE INTO template_asset_refs(owner_kind,owner_id,asset_id,created_at) VALUES('print',?,?,?)").bind(batchId, assetId, createdAt));
    }
  ...
  private async freezeItems(request: PrintRequest): Promise<PrintManifestV1["items"]> {
    ...
    if (request.kind === "qso") {
      ...
      const layout = AnyCardTemplateSchema.parse(JSON.parse(String(template.layout_json)));
      const assetRefs = layout.schema_version === 2 ? [...new Set(layout.elements.filter((element) => element.type === "image").map((element) => element.asset_id))] : [];
      if (assetRefs.length) {
        const assets = await this.env.DB.prepare(`SELECT COUNT(*) AS count FROM template_assets WHERE template_id=? AND id IN (${assetRefs.map(() => "?").join(",")})`).bind(request.template_id, ...assetRefs).first<{ count: number }>();
        if (Number(assets?.count ?? 0) !== assetRefs.length) throw new PrintBatchError("TEMPLATE_ASSET_MISSING", "Template references unavailable assets", 409);
      }
