...
import { enforceLookupLimit, enforcePublicLimit } from "../../platform/rate-limit";
...
const lookupSchema = z.object({
  call: z.string().trim().regex(/^[A-Za-z0-9]+(?:\/[A-Za-z0-9]+)*$/).min(4).max(16),
...
export function registerPublicRoutes(app: Hono<{ Bindings: Env; Variables: RequestVariables }>): void {
  ...
  app.get("/api/v1/public/cards/:publicId", async (c) => {
    ...
    if (raw.status === "void") {
      return problem(410, "https://myqsl.app/problems/void-card", "Card voided", "This card is no longer available", c.req.path);
    }
  ...
  app.get("/api/v1/public/cards/:publicId/image", async (c) => {
    ...
    // IMPORTANT: Status check precedes any conditional request handling.
    // A void card MUST return 410, NEVER 304!
    if (row.status === "void") {
      return problem(410, "https://myqsl.app/problems/void-card", "Card voided", "This card is no longer available", c.req.path);
    }
    if (!row.image_r2_key) {
    ...
    const etag = row.content_sha256 ? `"${row.content_sha256}"` : undefined;
  ...
  app.post("/api/v1/public/card-lookup", async (c) => {
    ...
    if (!parsed.success) {
      return problem(422, "https://myqsl.app/problems/validation", "Validation failed", "Exact call and UTC date are required", c.req.path);
    }
