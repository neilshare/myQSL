...
export function registerImportRoutes(app: Hono<{ Bindings: Env; Variables: RequestVariables }>): void {
  app.post("/api/v1/imports", async (c) => {
    ...
    catch (error) { return problem(422, "https://myqsl.app/problems/validation", "Validation failed", error instanceof Error ? error.message : "Invalid import job", c.req.path); }
  });
  app.post("/api/v1/imports/:id/chunks", async (c) => {
    ...
    catch (error) { const status = error instanceof ImportConflictError ? 409 : 422; return problem(status, `https://myqsl.app/problems/${status === 409 ? "conflict" : "validation"}`, status === 409 ? "Conflict" : "Validation failed", error instanceof Error ? error.message : "Invalid import chunk", c.req.path); }
  });
  app.post("/api/v1/imports/:id/complete", async (c) => {
    ...
    catch (error) { return problem(404, "https://myqsl.app/problems/not-found", "Not found", error instanceof Error ? error.message : "Import job not found", c.req.path); }
  });
}
