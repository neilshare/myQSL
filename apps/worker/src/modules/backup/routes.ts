...
export function registerBackupRoutes(app: Hono<{ Bindings: Env; Variables: RequestVariables }>): void {
  app.post("/api/v1/backups/run", async (c) => {
    if (!c.env.D1_BACKUP_WORKFLOW) return problem(503, "https://myqsl.app/problems/backup-unavailable", "Backup unavailable", "Workflow binding is not configured", c.req.path);
    if (await new BackupRepository(c.env.DB).running()) return problem(409, "https://myqsl.app/problems/backup-conflict", "Backup already running", "A backup workflow is already active", c.req.path);
    ...
    catch { return problem(409, "https://myqsl.app/problems/backup-conflict", "Backup already running", "A backup workflow is already active", c.req.path); }
  });
