...
export function registerBackupRoutes(app: Hono<{ Bindings: Env; Variables: RequestVariables }>): void {
  app.post("/api/v1/backups/run", async (c) => {
    ...
    if (await new BackupRepository(c.env.DB).running()) return problem(409, "https://eqsr.app/problems/backup-conflict", "Backup already running", "A backup workflow is already active", c.req.path);
