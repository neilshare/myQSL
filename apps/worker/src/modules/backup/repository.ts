...
export class BackupRepository {
  ...
  get(id: string): Promise<BackupRunRow | null> {
    return this.db.prepare("SELECT * FROM backup_runs WHERE id = ?").bind(id).first<BackupRunRow>();
  }
  running(): Promise<BackupRunRow | null> {
    return this.db.prepare("SELECT * FROM backup_runs WHERE status = 'running' ORDER BY started_at DESC LIMIT 1").first<BackupRunRow>();
  }
  latest(): Promise<BackupRunRow | null> {
    return this.db.prepare("SELECT * FROM backup_runs ORDER BY started_at DESC LIMIT 1").first<BackupRunRow>();
  }
  list(limit = 20): Promise<BackupRunRow[]> {
    return this.db.prepare("SELECT * FROM backup_runs ORDER BY started_at DESC LIMIT ?").bind(limit).all<BackupRunRow>().then((res) => res.results ?? []);
  }
  ...
  async markVerified(id: string, verifiedAt: number): Promise<BackupRunRow> {
    await this.db.prepare("UPDATE backup_runs SET verified_at = ? WHERE id = ?").bind(verifiedAt, id).run();
    return this.get(id) as Promise<BackupRunRow>;
  }
}
