...
export class CardBatchService {
  ...
  async create(input: Request, requestKey: string): Promise<{ id: string; card_ids: string[]; replayed: boolean }> {
    ...
    const layout = AnyCardTemplateSchema.parse(JSON.parse(String(template.layout_json))); const assetIds = layout.schema_version === 2 ? [...new Set(layout.elements.filter((element) => element.type === "image").map((element) => element.asset_id))] : [];
    if (assetIds.length) {
      const assets = await this.env.DB.prepare(`SELECT COUNT(*) AS count FROM template_assets WHERE template_id=? AND id IN (${assetIds.map(() => "?").join(",")})`).bind(input.template_id, ...assetIds).first<{ count: number }>();
      if (Number(assets?.count ?? 0) !== assetIds.length) throw new CardBatchError("TEMPLATE_ASSET_MISSING", "Template references unavailable assets", 409);
    }
    const batchId = `cards_${crypto.randomUUID()}`; const now = this.now(); const cardIds = input.qso_ids.map(() => nanoid(16));
