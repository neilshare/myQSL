...
export function ImportPage() {
  ...
  const [message, setMessage] = useState(locale === "zh" ? "请选择 ADIF 文件" : "Please select an ADIF file");
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setBusy(true);
    setMessage(locale === "zh" ? "正在解析与上传 ADIF 日志..." : "Parsing and uploading ADIF logs...");
    setProgress(null);
    ...
    try {
      ...
      setMessage(
        locale === "zh"
          ? `导入成功！共处理 ${result.total} 条通联记录（${result.chunks} 个分块）。`
          : `Import successful! Processed ${result.total} records (${result.chunks} chunks).`
      );
      if (result.counts) {
        setProgress({
      ...
      if (controller.signal.aborted || (error instanceof Error && /abort/i.test(error.message))) {
        setMessage(locale === "zh" ? "导入已取消" : "Import cancelled");
      } else {
        setMessage(error instanceof Error ? error.message : (locale === "zh" ? "导入失败" : "Import failed"));
      }
    } finally {
      setBusy(false);
  ...
  const handleCancel = () => {
    if (abortControllerRef.current) {
      ...
      setMessage(locale === "zh" ? "正在取消导入..." : "Cancelling import...");
    }
  };
  ...
  return (
    <section style={{ maxWidth: "680px", margin: "0 auto", padding: "0" }}>
      <div className="card-section" style={{ padding: "1.75rem" }}>
        <h2 style={{ marginTop: 0, fontSize: "1.5rem" }}>{t("import.title")}</h2>
        <p style={{ color: "var(--text-muted)", fontSize: "0.95rem", lineHeight: 1.5 }}>
          {t("import.subtitle")}
        </p>

        <div style={{ marginTop: "1.5rem", padding: "1.5rem", border: "2px dashed var(--border-subtle)", borderRadius: "8px", textAlign: "center", background: "var(--bg-input)" }}>
          <input
            ref={fileInputRef}
            type="file"
            accept=".adi,.adif"
        ...
        {progress && progress.totalRecords > 0 && (
          <div style={{ marginTop: "1.5rem" }}>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.875rem", color: "var(--text-muted)", marginBottom: "0.5rem" }}>
              <span>
                {locale === "zh"
                  ? `上传进度: ${progress.currentChunk} / ${progress.totalChunks} 块 (${progress.processedRecords} / ${progress.totalRecords} 条)`
                  : `Progress: ${progress.currentChunk} / ${progress.totalChunks} chunks (${progress.processedRecords} / ${progress.totalRecords} records)`}
              </span>
            ...
            <div
              ...
              aria-valuemin={0}
              aria-valuemax={100}
              style={{ width: "100%", height: "8px", background: "var(--control-pill-bg)", borderRadius: "4px", overflow: "hidden" }}
            >
              <div
                style={{
                  width: `${percentage}%`,
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: "0.75rem", marginTop: "1rem" }}>
              <div style={{ padding: "0.75rem", borderRadius: "6px", background: "rgba(16, 185, 129, 0.1)", border: "1px solid rgba(16, 185, 129, 0.3)", textAlign: "center" }}>
                ...
                <div style={{ fontSize: "1.25rem", fontWeight: "bold", color: "var(--success, #10b981)", marginTop: "0.25rem" }}>{progress.counts.ready}</div>
              </div>
              ...
              <div style={{ padding: "0.75rem", borderRadius: "6px", background: "rgba(59, 130, 246, 0.1)", border: "1px solid rgba(59, 130, 246, 0.3)", textAlign: "center" }}>
                ...
                <div style={{ fontSize: "1.25rem", fontWeight: "bold", color: "var(--accent-primary)", marginTop: "0.25rem" }}>{progress.counts.duplicate}</div>
              </div>
              <div style={{ padding: "0.75rem", borderRadius: "6px", background: "rgba(239, 68, 68, 0.1)", border: "1px solid rgba(239, 68, 68, 0.3)", textAlign: "center" }}>
                ...
                <div style={{ fontSize: "1.25rem", fontWeight: "bold", color: "var(--danger, #ef4444)", marginTop: "0.25rem" }}>{progress.counts.rejected}</div>
              </div>
            </div>
          </div>
        )}
        ...
        <p
          ...
          style={{
            marginTop: "1.25rem",
            color: busy ? "var(--accent-primary)" : message.includes("成功") || message.includes("successful") ? "var(--success)" : message.includes("取消") || message.includes("cancelled") ? "#fbbf24" : "var(--text-muted)",
            fontWeight: 500,
            fontSize: "0.95rem"
          }}
        >
